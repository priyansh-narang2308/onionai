...
import {
  Calendar as CalendarIcon, List, Plus, X, Clock, Sparkles,
  ChevronRight, FileText, CheckCircle, Send, Edit3,
...
export default function ScheduleTab() {
  ...
  const { data: postsData, isLoading, refetch, error } = useQuery({
    ...
    queryFn: () => fetchWithAuth(`/api/post?group_by_date=true${statusTab !== "all" ? `&status=${statusTab}` : ""}`, { method: "GET" }, getToken),
  })
  ...
  const { data: totalsData } = useQuery({
    ...
    queryFn: () => fetchWithAuth("/api/post/totals", { method: "GET" }, getToken),
  })
  ...
  const updatePostMutation = useMutation({
    mutationFn: (payload: { id: string; content: string; scheduledAt?: string; status?: string }) =>
      fetchWithAuth(`/api/post/${payload.id}`, {
        method: "PATCH",
        body: JSON.stringify({ content: payload.content, scheduledAt: payload.scheduledAt, status: payload.status }),
      }, getToken),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["posts"] })
      queryClient.invalidateQueries({ queryKey: ["post-totals"] })
  ...
  const publishNowMutation = useMutation({
    mutationFn: (postId: string) =>
      fetchWithAuth(`/api/post/${postId}/publish`, { method: "POST" }, getToken),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["posts"] })
      queryClient.invalidateQueries({ queryKey: ["post-totals"] })
  ...
  const handleSchedulePost = () => {
    if (!composerText.trim()) { toast("Content is required", "error"); return }
    if (!selectedChannelTypeId) { toast("Select a channel", "error"); return }
    if (!selectedTime) { toast("Select a time", "error"); return }
  ...
  const renderCalendarGrid = () => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      days.push(
        <TouchableOpacity
          ...
          style={[styles.calendarDayCell, isSelected && styles.calendarDayCellSelected, postsOnDay.length > 0 && styles.calendarDayCellHasPosts]}
        >
          <Text style={[styles.calendarDayText, isSelected && styles.calendarDayTextSelected, postsOnDay.length > 0 && styles.calendarDayTextHasPosts]}>
            {i}
          </Text>
    ...
    return (
      <View style={styles.calendarCard}>
        <Text style={styles.calendarMonthHeading}>{format(new Date(), "MMMM yyyy")}</Text>
        <View style={styles.calendarDayNamesRow}>
          {["S", "M", "T", "W", "T", "F", "S"].map((n, i) => (
            <Text key={i} style={styles.calendarDayNameText}>{n}</Text>
          ))}
        </View>
        <View style={styles.calendarGrid}>{days}</View>
        {selectedCalendarDay !== null && (
          <View style={styles.calendarDetailsSection}>
            <Text style={styles.calendarDetailsHeading}>Posts on {format(new Date(new Date().getFullYear(), new Date().getMonth(), selectedCalendarDay), "MMM d")}:</Text>
            {(() => {
              const postsOnDay = getPostsForDay(selectedCalendarDay)
              if (postsOnDay.length === 0) return <Text style={styles.noPostsOnDayText}>No posts for this date</Text>
              return postsOnDay.map(post => (
                <TouchableOpacity key={post.id} onPress={() => handleEditPost(post)} style={styles.timelinePostCardCompact}>
                  <View style={styles.timelineHeaderRow}>
                    <Text style={styles.timelineChannelName}>{post.user_channels?.channel_types?.name}</Text>
                    <View style={styles.statusPillActive}>
                      <Text style={styles.statusPillActiveText}>{post.status.toUpperCase()}</Text>
                    </View>
                  </View>
                  <Text style={styles.timelineBodyText} numberOfLines={2}>{post.content}</Text>
                </TouchableOpacity>
              ))
            })()}
          </View>
    ...
    <SafeAreaView style={styles.container} edges={["top", "left", "right"]}>
      <View style={styles.header}>
        ...
        <View style={styles.viewModeRow}>
          <TouchableOpacity onPress={() => setViewMode("list")} style={[styles.viewModeBtn, viewMode === "list" && styles.viewModeBtnActive]}>
            <List color={viewMode === "list" ? "#84cc16" : "#71717a"} size={16} />
          </TouchableOpacity>
          <TouchableOpacity onPress={() => setViewMode("calendar")} style={[styles.viewModeBtn, viewMode === "calendar" && styles.viewModeBtnActive]}>
        ...
        <View style={styles.statusTabBar}>
          {STATUS_TABS.map((tab) => {
            const count = tab.key === "all"
              ? (totals.draft || 0) + (totals.queue || 0) + (totals.published || 0) + (totals.failed || 0)
              : (totals[tab.key] || 0)
            return (
              <TouchableOpacity
                key={tab.key}
              ...
              >
                <Text style={[styles.statusTabText, statusTab === tab.key && styles.statusTabTextActive]}>
                  {tab.label}
                </Text>
                <Text style={[styles.statusTabCount, statusTab === tab.key && styles.statusTabCountActive]}>
                  {count}
                </Text>
              </TouchableOpacity>
            )
        ...
        <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          {groupPosts.length === 0 ? (
            <View style={styles.emptyFeedCard}>
              ...
              <Text style={styles.emptyFeedDesc}>Tap + to create your first post</Text>
            </View>
          ) : (
            groupPosts.map((group) => (
              <View key={group.key} style={styles.groupSection}>
                ...
                {group.posts.map((post) => (
                  <TouchableOpacity key={post.id} onPress={() => handleEditPost(post)} style={styles.timelinePostCard}>
                    <View style={styles.timelineHeaderRow}>
                      <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                        <View style={[styles.channelTypeBadge, { backgroundColor: post.user_channels?.channel_types?.color || "#09090b" }]}>
                      ...
                      <View style={[styles.statusPill, post.status === "draft" && styles.statusPillDraft, post.status === "queue" && styles.statusPillQueue, post.status === "published" && styles.statusPillPublished, post.status === "failed" && styles.statusPillFailed]}>
                        <Text style={[styles.statusPillText, post.status === "draft" && styles.statusPillTextDraft, post.status === "queue" && styles.statusPillTextQueue, post.status === "published" && styles.statusPillTextPublished, post.status === "failed" && styles.statusPillTextFailed]}>
                          {post.status.toUpperCase()}
                        </Text>
                      </View>
                    </View>
                    <Text style={styles.timelineBodyText} numberOfLines={3}>{post.content}</Text>
                    <View style={styles.timelineFooter}>
                      <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                        <Clock color="#71717a" size={14} />
                        <Text style={styles.timelineTimeText}>{format(new Date(post.scheduled_at), "MMM d, h:mm a")}</Text>
                      </View>
                      {post.status === "queue" && (
                        <TouchableOpacity onPress={() => handlePublishNow(post.id)} style={styles.publishNowBtn}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <View style={styles.modalFooter}>
              <TouchableOpacity onPress={handleSchedulePost} style={styles.scheduleActionBtn} disabled={createPostMutation.isPending}>
                {createPostMutation.isPending ? <ActivityIndicator color="#ffffff" size="small" /> : (
                  <><CheckCircle color="#ffffff" size={16} /><Text style={styles.scheduleActionBtnText}>Schedule</Text></>
                )}
              </TouchableOpacity>
            </View>
          </View>
            ...
            <ScrollView style={styles.modalFormScroll} showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.inputAreaCard}>
                <TextInput
                  ...
                  onChangeText={(text) => setEditingPost(prev => prev ? { ...prev, content: text } : null)}
                  multiline
                  numberOfLines={4}
                />
              </View>
...
const styles = StyleSheet.create({
  ...
  viewModeRow: { flexDirection: "row", backgroundColor: "#f4f4f5", borderRadius: 10, padding: 2, borderWidth: 0.5, borderColor: "#e4e4e7" },
  viewModeBtn: { padding: 8, borderRadius: 8 },
  viewModeBtnActive: { backgroundColor: "#ffffff" },
  statusTabBar: { flexDirection: "row", paddingHorizontal: 16, paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: "#f4f4f5", gap: 6 },
  statusTab: { flexDirection: "row", alignItems: "center", paddingVertical: 6, paddingHorizontal: 12, borderRadius: 16, backgroundColor: "#f4f4f5", gap: 4 },
  statusTabActive: { backgroundColor: "#f4f5f0", borderWidth: 1, borderColor: "#e4e6d9" },
  ...
  groupLabel: { fontSize: 11, fontWeight: "800", color: "#a1a1aa", marginBottom: 12, letterSpacing: 0.5 },
  timelinePostCard: { backgroundColor: "#ffffff", borderWidth: 1, borderColor: "#e4e4e7", borderRadius: 20, padding: 16, marginBottom: 12 },
  timelineHeaderRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 12 },
  channelTypeBadge: { width: 32, height: 32, borderRadius: 8, alignItems: "center", justifyContent: "center" },
  ...
  statusPillActive: { backgroundColor: "#f4f5f0", borderWidth: 0.5, borderColor: "#e4e6d9", paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 },
  ...
  timelineFooter: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginTop: 12, borderTopWidth: 1, borderTopColor: "#f4f4f5", paddingTop: 10 },
  timelineTimeText: { fontSize: 11, color: "#71717a", fontWeight: "600" },
  publishNowBtn: { flexDirection: "row", alignItems: "center", backgroundColor: "#84cc16", paddingHorizontal: 10, paddingVertical: 5, borderRadius: 8, gap: 4 },
  ...
  label: { fontSize: 11, fontWeight: "700", color: "#09090b", marginBottom: 8, marginTop: 12, textTransform: "uppercase", letterSpacing: 0.5 },
  channelRowSelector: { flexDirection: "row", flexWrap: "wrap", gap: 8, marginBottom: 10 },
  channelSelectBadge: { flexDirection: "row", alignItems: "center", backgroundColor: "#f4f4f5", paddingHorizontal: 12, paddingVertical: 8, borderRadius: 12, borderWidth: 1, borderColor: "#e4e4e7", gap: 6 },
  channelSelectBadgeActive: { backgroundColor: "#f4f5f0", borderColor: "#84cc16" },
  avatarDot: { width: 8, height: 8, borderRadius: 4 },
  channelSelectText: { fontSize: 12, fontWeight: "600", color: "#71717a" },
  inputAreaCard: { backgroundColor: "#f4f4f5", borderRadius: 16, borderWidth: 1, borderColor: "#e4e4e7", padding: 12, marginBottom: 10 },
  composerInput: { height: 100, fontSize: 14, color: "#09090b", textAlignVertical: "top", lineHeight: 20 },
  inputMeta: { borderTopWidth: 0.5, borderTopColor: "#e4e4e7", paddingTop: 8, marginTop: 8, alignItems: "flex-end" },
  charCountText: { fontSize: 10, color: "#71717a" },
  modalFooter: { marginTop: 10, borderTopWidth: 1, borderTopColor: "#f4f4f5", paddingTop: 16 },
  scheduleActionBtn: { backgroundColor: "#84cc16", flexDirection: "row", alignItems: "center", justifyContent: "center", paddingVertical: 14, borderRadius: 14, gap: 8 },
  scheduleActionBtnText: { fontSize: 13, fontWeight: "700", color: "#ffffff" },
  publishNowLargeBtn: { backgroundColor: "#84cc16", flexDirection: "row", alignItems: "center", justifyContent: "center", paddingVertical: 14, borderRadius: 14, gap: 8, marginTop: 12 },
  publishNowLargeBtnText: { fontSize: 13, fontWeight: "700", color: "#ffffff" },
  calendarCard: { backgroundColor: "#ffffff", borderWidth: 1, borderColor: "#e4e4e7", borderRadius: 24, padding: 20 },
  calendarMonthHeading: { fontSize: 16, fontWeight: "800", color: "#09090b", textAlign: "center", marginBottom: 16 },
  calendarDayNamesRow: { flexDirection: "row", justifyContent: "space-between", marginBottom: 10 },
  calendarDayNameText: { width: "13%", textAlign: "center", fontSize: 11, fontWeight: "700", color: "#a1a1aa" },
  calendarGrid: { flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between", rowGap: 10 },
  calendarDayCell: { width: "13%", height: 38, alignItems: "center", justifyContent: "center", borderRadius: 10, backgroundColor: "#f4f4f5", position: "relative" },
  calendarDayCellSelected: { backgroundColor: "#84cc16" },
  calendarDayCellHasPosts: { backgroundColor: "#f4f5f0", borderWidth: 1, borderColor: "#e4e6d9" },
  ...
  calendarPostDot: { position: "absolute", bottom: 4, width: 4, height: 4, borderRadius: 2, backgroundColor: "#84cc16" },
  calendarDetailsSection: { marginTop: 20, borderTopWidth: 1, borderTopColor: "#f4f4f5", paddingTop: 16 },
  calendarDetailsHeading: { fontSize: 13, fontWeight: "700", color: "#09090b", marginBottom: 12 },
  noPostsOnDayText: { fontSize: 12, color: "#a1a1aa", textAlign: "center", paddingVertical: 10 },
  timelinePostCardCompact: { backgroundColor: "#f4f4f5", borderRadius: 14, padding: 12, marginBottom: 8 },
})
