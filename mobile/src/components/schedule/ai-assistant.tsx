    ...
    try {
      const resp = await fetchWithAuth(
        "/api/post/generate-post",
        {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            prompt: `Custom: ${customPrompt}. Original content: ${content}`,
            action: "custom",
          }),
        },
        getToken,
        ...
        {actions.map(({ id, label, icon: Icon }) => (
          <TouchableOpacity
            ...
            style={{
              ...
              paddingVertical: 10,
              borderRadius: 12,
              backgroundColor: loading === id ? "#f0fdf4" : "#ffffff",
              borderWidth: 1,
              borderColor: loading === id ? "#84cc16" : "#e4e4e7",
            }}
          >
            {loading === id ? (
            ...
            <Text
              style={{
                fontSize: 13,
                fontWeight: "600",
                color: loading === id ? "#4d7c0f" : "#09090b",
              }}
            >
              {label}
            </Text>
