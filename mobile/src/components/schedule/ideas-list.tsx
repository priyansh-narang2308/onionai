  ...
  const loadIdeas = React.useCallback(async () => {
    ...
    try {
      const data = await fetchWithAuth(
        `/api/idea${selectedGroup ? `?group=${selectedGroup}` : ""}`,
        { method: "GET" },
        getToken,
      );
    ...
    <View>
      ...
      >
        ...
        <TouchableOpacity onPress={loadIdeas} style={{ padding: 4 }}>
          <Text style={{ fontSize: 12, color: "#84cc16", fontWeight: "600" }}>
            Refresh
          </Text>
        </TouchableOpacity>
      </View>
        ...
        {groups.map((g) => (
          <TouchableOpacity
            ...
            style={{
              ...
              borderRadius: 20,
              backgroundColor:
                selectedGroup === g ? GROUP_COLORS[g] || "#09090b" : "#f4f4f5",
            }}
          >
            <Text
              style={{
      ...
      {loading ? (
        ...
        >
          ...
          <Text style={{ fontSize: 13, color: "#a1a1aa", marginTop: 8 }}>
            No ideas yet. Generate some ideas first.
          </Text>
        </View>
      ) : (
        <View style={{ gap: 8 }}>
          {ideas.slice(0, 5).map((idea: Idea) => (
            ...
            >
              <Text
                style={{ fontSize: 14, color: "#09090b", lineHeight: 20 }}
                numberOfLines={3}
              >
                ...
                {idea.group && (
                  <View
                    style={{
                      ...
                      borderRadius: 10,
                      backgroundColor:
                        (GROUP_COLORS[idea.group] || "#09090b") + "20",
                    }}
                  >
                    <Text
                      style={{
                        fontSize: 10,
                        fontWeight: "600",
                        color: GROUP_COLORS[idea.group] || "#09090b",
                        textTransform: "capitalize",
                      }}
                    >
                      {idea.group}
