...
export function EditPostDialog({ visible, onClose, post }: Props) {
  ...
  useEffect(() => {
    if (post) {
      ...
      setScheduleDate(
        post.scheduled_at ? post.scheduled_at.substring(0, 10) : "",
      );
      setScheduleTime(
        post.scheduled_at ? post.scheduled_at.substring(11, 16) : "",
      );
                ...
                return (
                  <View
                    ...
                    style={{
                      borderWidth: 1,
                      borderColor:
                        expandedChannel === ch ? info?.color + "40" : "#f4f4f5",
                      borderRadius: 14,
                      overflow: "hidden",
                    }}
                  >
                    <TouchableOpacity
                      onPress={() =>
                        setExpandedChannel(expandedChannel === ch ? null : ch)
                      }
                      style={{
                        flexDirection: "row",
                        alignItems: "center",
